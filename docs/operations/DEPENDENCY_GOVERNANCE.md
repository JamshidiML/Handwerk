...
Platform regression tests require that exact manifest and lock record, the Tailwind graph, and the existing `lightningcss-linux-x64-gnu@1.31.1` record. A disposable Noble/glibc x86_64 container proved that one clean install materializes both bindings, and hosted PR run [`31699278297`](https://github.com/JamshidiML/Handwerk/actions/runs/31699278297) completed the cold build.
